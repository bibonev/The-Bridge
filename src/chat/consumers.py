...
def ws_receive(message):
    ...
    if set(data.keys()) != set(('handle', 'handle_type', 'message')):
        log.debug("ws message unexpected format data=%s", data)
        return

    print("New data: ", data)
    if data:
        log.debug('chat message conversation=%s handle=%s handle_type=%s message=%s', 
            conversation.label, data['handle'], data['handle_type'], data['message'])
